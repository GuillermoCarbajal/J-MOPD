...
def save_motion_flow(offsets, img_name='motion_flow', title='motion flow'):

    '''
    offsets: P,2,H,W
    '''
    P,D,H,W = offsets.shape
    ...
    plt.scatter(xs, ys,marker='x',s=1,c='blue')
        
    warped_grid = grid + torch.flip(offsets,dims=[3])  #  P, H, W, 2

    xs = warped_grid[:,33::65,33::65, 0].detach().cpu().numpy().flatten()
    ys = warped_grid[:,33::65,33::65, 1].detach().cpu().numpy().flatten()
    xs_f = xs[(xs < W+100) * (ys <H+100)*(xs > -100) * (ys >-100)]
    ys_f = ys[(xs < W+100) * (ys <H+100)*(xs > -100) * (ys >-100)]

    colors = np.arange(len(xs_f))/len(xs_f)
    #plt.scatter(xs_f, ys_f,marker='x',s=1, c='red')
    plt.scatter(xs_f, ys_f,marker='x',s=1, c=colors)

    plt.gca().invert_yaxis()  # y-axis is positive upwards in plot() but it is positive downwards in the image
    plt.grid()
    plt.xlim(0,W)
    plt.ylim(H,0)
    plt.axis('equal')
